...
public class NetworkObserverSoldier: Soldier {
    ...
    public init() {
        name = "流量"
        team = "性能检测"
        icon = ImageManager.imageWithName("VVTools_icon_network")
        monitor = ANRMonitor()
        cache.countLimit = responseCacheByteLimit
    }
    ...
    func start() {
        ...
        URLProtocol.registerClass(VVToolsURLProtocol.self)
    ...
    func end() {
        ...
        URLProtocol.unregisterClass(VVToolsURLProtocol.self)
        isActive = false
    }
    ...
    func recordRequest(request: URLRequest, response: URLResponse?, responseData: Data?, error: NSError?, startDate: Date) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: NSNotification.Name.VVToolsNetworkObserverSoldierNewFlowDidReceive, object: flowModel)
        }
    }
}
