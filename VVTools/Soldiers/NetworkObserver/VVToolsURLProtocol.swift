...
private let KVVToolsURLProtocolIdentifier = "KVVToolsURLProtocolIdentifier"

class VVToolsURLProtocol: URLProtocol, URLSessionDataDelegate {
    ...
    override class func canInit(with request: URLRequest) -> Bool {
        if URLProtocol.property(forKey: KVVToolsURLProtocolIdentifier, in: request) != nil {
            return false
        }
    ...
    override class func canonicalRequest(for request: URLRequest) -> URLRequest {
        ...
        URLProtocol.setProperty("VVTools", forKey: KVVToolsURLProtocolIdentifier, in: mutableRequest)
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, willPerformHTTPRedirection response: HTTPURLResponse, newRequest request: URLRequest, completionHandler: @escaping (URLRequest?) -> Void) {
        ...
        // us anyway.
        URLProtocol.removeProperty(forKey: KVVToolsURLProtocolIdentifier, in: redirectRequest)
    ...
    func urlSession(_ session: URLSession, didReceive challenge: URLAuthenticationChallenge, completionHandler: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        client?.urlProtocol(self, didReceive: URLAuthenticationChallenge(authenticationChallenge: challenge, sender: VVToolsURLSessionChallengeSender(completionHandler: completionHandler)))
    }
...
extension URLSession {
    ...
    @objc convenience init(captainConfiguration: URLSessionConfiguration, delegate: URLSessionDelegate?, delegateQueue queue: OperationQueue?) {
        ...
        let result = captainConfiguration.protocolClasses?.contains(where: { (type) -> Bool in
            if type == VVToolsURLProtocol.self {
                return true
            }else {
                return false
            }
        ...
        if result != true {
            ...
            protocols?.insert(VVToolsURLProtocol.self, at: 0)
            captainConfiguration.protocolClasses = protocols
        }
    }
}

class VVToolsURLSessionChallengeSender: NSObject, URLAuthenticationChallengeSender {
