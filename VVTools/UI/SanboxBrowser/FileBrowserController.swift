...
open class FileBrowserController: UIViewController {
...
extension FileBrowserController: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        do {
            let attributes = try FileManager.default.attributesOfItem(atPath: fullPath)
            if attributes[FileAttributeKey.type] as? String == FileAttributeType.typeDirectory.rawValue {
                let fileBrowserController = FileBrowserController(path: fullPath)
                ...
                if TableListViewController.supportsExtension(fileExtension) {
                    let vc = TableListViewController(filePath: fullPath)
                    navigationController?.pushViewController(vc, animated: true)
                }else if FilePreviewViewController.supportsExtension(fileExtension) {
                    let previewVC = FilePreviewViewController(filePath: fullPath)
