...
class TableContentViewController: UIViewController {
    ...
    init(filePath: String, tableName: String) {
        ...
        connector = DatabaseConnector(path: filePath)
...
extension TableContentViewController: ExcelViewDelegate {
    func excelView(_ excelView: ExcelView, didTapGridWith content: String) {
        let vc = TextPreviewViewController(text: content)
...
extension TableContentViewController: ExcelViewDataSource {
